...
## Запуск
Создайте файл .env по примеру файла env.example(представлен в директории проекта) в директорию avito-intern, затем для запуска воспользуйтесь командой `docker-compose up`, после чего начнет свою работу сервер на порту 8080 и создастся бд с миграцией в ней. Для доступа к админским ручкам воспользуйтесь токеном: admin-secret-token-123, а для юзерских: user-secret-token-456. (или тот который вы укажете в значениях ADMIN_TOKEN и USER_TOKEN в .env)

## Вопросы по разработке
При ознакомлении с заданием и openapi, столкнулся с тем, что мало ошибок проверяется и ситуаций, были добавлены другие статусы ошибок и поэтому был сделан сваггер, со всей информацией сервера. Сам сваггер открывается по адресу http://localhost:8080/swagger/index.html после того как запустите сервер.
Для токенов нужно писать в хедере Authorization токен формата Bearer {token}. Сам токен сравнивается со значением из .env. Хотел делать полноценную jwt авторизацию но данная логика выходит из задания.
...
## Линтер
Для запуска линтера используйте команду `golangci-lint run`
