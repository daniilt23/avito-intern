...
## Запуск
Создайте файл .env по примеру файла env.example(представлен в директории проекта) в директорию avito-intern, затем
Для запуска воспользуйтесь командой `docker-compose up`, после чего начнет свою работу сервер на порту 8080 и создастся бд с миграцией в ней. Для доступа к админским ручкам воспользуйтесь токеном: admin-secret-token-123, а для юзерских: user-secret-token-456. (или тот который вы укажате в значениях ADMIN_TOKEN и USER_TOKEN в .env)
